...
class Environment(object):
    def __init__(self):
        self.num_links = 18
        self.link_capacity = np.array([4, 6, 6, 4, 1, 3, 6, 6, 3, 1, 3, 6, 6, 3, 4, 6, 6, 4], dtype=np.float32) * 100.0
        self.link_names = ["A1", "A2", "B2", "B3", "16", "14", "24", "25", "35", "38", "46", "47", "57", "58",
                           "6C", "7C", "7D", "8D"]

        self.controllable_sd_list = ["AC", "AD", "BC", "BD"]

        self.num_paths = [6, 4, 4, 6]
        self.paths = {
            "AC": [['A', '1', '6', 'C'], ['A', '1', '4', '6', 'C'], ['A', '1', '4', '7', 'C'],
                   ['A', '2', '4', '6', 'C'], ['A', '2', '4', '7', 'C'], ['A', '2', '5', '7', 'C']],
            "AD": [['A', '1', '4', '7', 'D'], ['A', '2', '4', '7', 'D'], ['A', '2', '5', '7', 'D'],
                   ['A', '2', '5', '8', 'D']],
            "BC": [['B', '2', '4', '6', 'C'], ['B', '2', '4', '7', 'C'], ['B', '2', '5', '7', 'C'],
                   ['B', '3', '5', '7', 'C']],
            "BD": [['B', '2', '4', '7', 'D'], ['B', '2', '5', '7', 'D'], ['B', '2', '5', '8', 'D'],
                   ['B', '3', '5', '7', 'D'], ['B', '3', '5', '8', 'D'], ['B', '3', '8', 'D']]
        }
        ...
        self.flows = np.array([self.flow_AC, self.flow_AD, self.flow_BC, self.flow_BD])
    ...
    def _get_dnn_state(self, link_state, global_state):
        ...
        for i, agent in enumerate(self.controllable_sd_list):
            feature = []
            paths = self.paths_mask[agent]
            for j in range(self.num_paths[i]):
                idx = np.where(paths[j, :] == 1.0)
                ft = np.copy(link_state[idx, :]).flatten()
                ft = np.concatenate([ft,global_state])
                feature.extend(list(ft))
            agent_obs.append(feature)
        return agent_obs


if __name__ == "__main__":
    env = Environment()
    # print(env.paths_mask)

    pointer = 100
    # flows = env.sample(pointer)
    # print(env.get_actions(flows))
    obs = env.reset()
    print(obs)
    print(len(obs))
    for i in range(len(env.controllable_sd_list)):
        print(len(obs[i]))
    # print(len(obs[0]))
    actions = []
    ...
    #     print(rewards, dnn_feature, done, info)
    # # print(env.get_input_features(flows))\
    # print(env.gen_data_dnn(1))
    # rewards = env.eval_best_actions()
    # print(rewards)
    # print(sum(rewards))
    # num_ac = len(range(env.action_effective_step, env.point_count - env.action_effective_step,
    #                    env.action_effective_step))
    # actionss_all = []
    # for a in range(num_ac):
    #     actionss_all.append([[1 / env.num_paths] * env.num_paths] * 4)
    # rew = env.eval_actions_requestnet(np.array(actionss_all))
    # print(sum(rew))
